...
router.post('/', async (req, res) => {
  try {
    ...
    // Check for duplicates before creating (core fields only)
    // Use case-insensitive search and trimmed values for better accuracy
    const isDuplicate = await DataEntry.findOne({
      year: data.year,
      eventType: { $regex: new RegExp(`^${String(data.eventType || '').trim()}$`, 'i') },
      eventCategory: { $regex: new RegExp(`^${String(data.eventCategory || '').trim()}$`, 'i') },
      eventName: { $regex: new RegExp(`^${String(data.eventName || '').trim()}$`, 'i') },
      startDate: data.startDate,
      venuePlace: { $regex: new RegExp(`^${String(data.venuePlace || '').trim()}$`, 'i') }
    });

    if (isDuplicate) {
      return res.status(409).json({
        message: 'A record with these exact core details already exists. Please use the "Save Anyway" option in the form if this is a distinct entry.',
        isDuplicate: true
      });
    }
...
// Delete multiple records by IDs (Undo Import feature)
router.post('/bulk-delete', async (req, res) => {
  try {
    ...
    // Optional: Add year lock check for each record if needed, 
    // but since these were just added in the session, they shouldn't be locked.
    
    const result = await DataEntry.deleteMany({ _id: { $in: ids } });
    res.json({ message: `Successfully deleted ${result.deletedCount} records`, count: result.deletedCount });
  } catch (error) {
    res.status(500).json({ message: error.message });
  }
});
